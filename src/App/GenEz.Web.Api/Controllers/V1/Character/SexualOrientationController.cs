...
{
    public class SexualOrientationController : ApiControllerBase
    {
        private readonly ICharacterQueryHandler _characterQueryHandler;

        public SexualOrientationController(
            ILogger<SexualOrientationController> logger,
        ...
        [AllowAnonymous]
        [HttpGet]
        public async Task<ActionResult<ApiResponseWithResult<IEnumerable<SexualOrientationDto>>>> GetAll()
        {
            return Respond(await _characterQueryHandler.GetAllSexOrientationsAsync());
        }
    }
}
